...
module.exports.printResults = function printResults(testResults /*: Map<string, *>*/) {
  //$off
 ...
 `);
  for (const result of testResults.values()) {
    const total = doDiffWithFloor.total ? result.heapTotal - floor.heapTotal : result.heapTotal;
    const used = doDiffWithFloor.used ? result.heapUsed - floor.heapUsed : result.heapUsed;
    const rss = doDiffWithFloor.rss ? result.rss - floor.rss : result.rss;
    const external = doDiffWithFloor.external ? result.external - floor.external : result.external;
